...
def analyze(data, height_cm, gender="ذكر"):
    ...
    with warnings.catch_warnings():
        ...
        return _analyze(data, height_cm, gender)
...
def _analyze(data, height_cm, gender):
    ...
    # --- اجتياز العارضة: أطول فترة يكون فيها الجسم شبه أفقي ---
    min_len = max(2, int(fps * 0.08))
    runs = [r for r in _runs(np.nan_to_num(trunk_s) > 55) if r[1] - r[0] + 1 >= min_len]
    if runs:
        a, b = max(runs, key=lambda r: (r[1] - r[0], np.nanmean(trunk_s[r[0]:r[1] + 1])))
    ...
    clear_ev = clearance if clearance != peak else min(n - 1, _at(t, t[peak] + 0.2))
    events = {
