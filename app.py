...
# تحقق من أن ملف pose_analysis.py محدّث (الإصدار الجديد يحتوي على تعويض حركة الكاميرا)
if not hasattr(pa, "reject_outliers"):
    st.error(
        "⚠️ ملف pose_analysis.py في المستودع قديم. ارفع النسخة الجديدة إلى GitHub "
        "ثم اضغط Manage app ← ⋮ ← Reboot app."
    )
    st.stop()
st.markdown(
    """
...
ملاحظات على ظروف التصوير (القيم "null" لم تُحسب لهذا السبب):
...
if st.button("🚀 بدء التحليل", width="stretch"):
    ...
    try:
        ...
        rows = [{"المؤشر": LABELS[k][0], "القيمة": "غير متاح" if v is None else v, "الوحدة": LABELS[k][1]}
                for k, v in result["metrics"].items() if k in LABELS]
        ...
        st.caption("تأكد أن كل صورة تطابق اسمها. إذا لم تطابق، فالقياسات المبنية عليها غير دقيقة.")
        ...
        # الرسوم: نعرض فترة القفزة فقط (من ثانية ونصف قبل الارتكاز حتى نصف ثانية بعد الاجتياز)
        s = result["series"]
        et = result["event_times"]
        cols_data = {
            "الزمن (ث)": s["t"],
            "زاوية ركبة الارتقاء (°)": s["knee_take"],
            "ميل الجذع عن العمودي (°)": s.get("trunk", s["knee_take"] * float("nan")),
        }
        if s.get("hip_height_cm") is not None:
            cols_data["ارتفاع الورك (سم)"] = s["hip_height_cm"]
        chart = pd.DataFrame(cols_data)
        chart = chart[(chart["الزمن (ث)"] >= et["plant"] - 1.5) & (chart["الزمن (ث)"] <= et["clearance"] + 0.5)]
        chart = chart.set_index("الزمن (ث)")
        g1, g2 = st.columns(2)
        g1.markdown(f"**زاوية ركبة الارتقاء** (الارتكاز {et['plant']}–{et['takeoff']} ث)")
        g1.line_chart(chart[["زاوية ركبة الارتقاء (°)"]])
        if "ارتفاع الورك (سم)" in chart:
            g2.markdown("**ارتفاع الورك (مسار مركز الثقل التقريبي)**")
            g2.line_chart(chart[["ارتفاع الورك (سم)"]])
        else:
            g2.markdown("**ميل الجذع عن العمودي** (90° = الجسم أفقي فوق العارضة)")
            g2.line_chart(chart[["ميل الجذع عن العمودي (°)"]])
